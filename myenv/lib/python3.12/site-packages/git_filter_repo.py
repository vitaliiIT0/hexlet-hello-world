...
class AncestryGraph(object):
    ...
    def get_parent_hashes(self, commit_hash):
        ...
        parent_fast_export_ids = [
            self._reverse_value[x] for x in parent_graph_ids
        ]
...
EXAMPLES
    ...
    For more detailed examples and explanations, see
      https://htmlpreview.github.io/?https://github.com/newren/git-filter-repo/blob/docs/html/git-filter-repo.html#EXAMPLES"""
        ...
        sdr = parser.add_argument_group(
            title=_("Sensitive Data Removal Handling")
        )
        sdr.add_argument(
...
class RepoFilter(object):
        ...
        def handle(which, args=None):
            ...
            if code_string:
                ...
                if "return " not in code_string and which not in (
                    ...
                    raise SystemExit(
                        _("Error: --%s-callback should have a return statement")
                        % which
                    )
    ...
    def _run_sanity_checks(self):
      ...
      Error: Cannot specify --sensitive-data-removal on a follow-up invocation
        ...
        if self._args.sensitive_data_removal:
            if self._args.stdin:
                msg = _(
                    "Error: sensitive data removal is incompatible with --stdin"
                )
                raise SystemExit(msg)
        ...
        if not self._args.force and not self._already_ran:
            ...
            RepoFilter.sanity_check(
                self._orig_refs, is_bare, self._config_settings
            )
            os.chdir(cwd)
    ...
    def _setup_lfs_orphaning_checks(self):
        ...
        if not self._already_ran:
            ...
            a = self._file_info_value.get_contents_by_identifier(
                b"HEAD:.gitattributes"
            )
        ...
        self._parser._lfs_object_tracker = (
        ...
        if self._already_ran:
            with open(lfs_objects_file, "br") as f:
                for line in f:
                    self._lfs_object_tracker.source_objects.objects.add(
                        line.strip()
                    )
    ...
    def loose_objects_are_replace_refs(git_dir, refs, num_loose_objects):
        ...
        for root, dirs, files in os.walk(object_dir):
            for filename in files:
                ...
                if objname not in replace_objects and validobj_re.match(
                    objname
                ):
                    return False
    ...
    def cleanup(
        self, repo, repack, reset, run_quietly=False, show_debuginfo=False
    ):
        ...
        for cmd in cleanup_cmds:
            if show_debuginfo:
                print(
                    "[DEBUG] Running{}: {}".format(location_info, " ".join(cmd))
                )
    ...
    def _flush_renames(self, old_hash=None, limit=0):
        ...
        while self._pending_renames:
            orig_hash, new_fast_export_id = self._pending_renames.popitem(
                last=False
            )
    ...
    def _tweak_commit(self, commit, aux_info):
        ...
        parents, new_1st_parent = self._maybe_trim_extra_parents(
        ...
        if not commit.dumped:
            if not self._prunable(
                ...
                if (
                    commit.branch.startswith(b"refs/")
                    or commit.branch == b"HEAD"
                ):
    ...
    def _migrate_origin_to_heads(self):
        ...
        if (
            ...
            and self._config_settings.get(b"remote.origin.mirror", b"false")
            != b"true"
        ):
            ...
            if refs_to_warn_about:
                msg = (
                    ...
                    + "\n            ".join(
                        [decode(x) for x in refs_to_warn_about]
                    )
                    + "\n"
        ...
        url = self._config_settings[b"remote.origin.url"].decode(
            errors="replace"
        )
        m = (
            _(
    ...
    def _compute_metadata(self, metadata_dir, orig_refs):
        ...
        if not self._already_ran:
            commit_renames = {
                old: new for old, new in self._commit_renames.items()
            }
        else:
        ...
        if not self._already_ran:
            ...
            old_commit_unrenames = dict(
                (v, k) for (k, v) in old_commit_renames.items()
            )
        ...
        if self._already_ran:
            ...
            with open(
                os.path.join(metadata_dir, b"first-changed-commits"), "br"
            ) as f:
                for line in f:
                    changed_commit, undeleted_self_or_ancestor = (
                        line.strip().split()
                    )
                    old_first_changes[changed_commit] = (
                        undeleted_self_or_ancestor
                    )
        ...
        for old, undeleted_self_or_ancestor in old_first_changes.items():
            ...
            if intermediate == deleted_hash:
                ...
                first_changes[old] = (
                    new_ancestor  # or usoa, since new_ancestor == usoa
                )
                continue
            assert (
                usoa == intermediate
            )  # old wasn't pruned => usoa == intermediate
    ...
    def _handle_lfs_metadata(self, metadata_dir):
        ...
        with open(
            os.path.join(metadata_dir, b"original_lfs_objects"), "bw"
        ) as f:
    ...
    def _record_metadata(self, metadata_dir, orig_refs):
        ...
        if self._args.sensitive_data_removal:
            ...
            print(
                f"You rewrote {changed_commits} (of {len(commit_renames)}) commits."
            )
            ...
            with open(
                os.path.join(metadata_dir, b"sensitive_data_removal"), "bw"
            ) as f:
        ...
        with open(
            os.path.join(metadata_dir, b"first-changed-commits"), "bw"
        ) as f:
            for commit, undeleted_self_or_ancestor in sorted(
                first_changes.items()
            ):
